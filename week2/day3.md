...
> ## 2026 implementation update — read this first
>
> Amazon Bedrock now has two different experiences that are easy to confuse:
>
> - **Projects / Workbench (`bedrock-mantle`)** is a console-first environment. A model appearing or responding there confirms that your account and that Region can use it, but it does **not** configure the Lambda application.
> - **Bedrock Runtime (`bedrock-runtime`)** is what this project uses from Lambda through `boto3`. The Lambda execution role, the Lambda Region, the runtime model ID, and the deployed code must all be configured independently.
>
> Do not infer runtime availability from the Workbench model picker. Verify it instead:
>
> ```bash
> aws bedrock list-foundation-models --region eu-north-1 --by-output-modality TEXT
> aws bedrock list-inference-profiles --region eu-north-1 --type-equals SYSTEM_DEFINED
> ```
>
> ### Verified starting choices
>
> | Need | Region | Runtime model ID | Notes |
> | --- | --- | --- | --- |
> | Lowest-risk EU Nova setup | `eu-north-1` | `eu.amazon.nova-2-lite-v1:0` | EU cross-Region inference profile; Bedrock selects a suitable EU Region. |
> | Keep processing in Stockholm | `eu-north-1` | `amazon.nova-2-lite-v1:0` | In-Region Nova 2 Lite. |
> | DeepSeek V3.2 in Stockholm | `eu-north-1` | `deepseek.v3.2` | In-Region model; use the runtime API, not a cross-Region profile. |
>
> Never add whitespace around `BEDROCK_MODEL_ID`. A value such as `amazon.nova-lite-v1:0 ` fails model resolution.
>
> ### Fresh deployment order
>
> 1. Create the memory and frontend buckets in the selected Region; copy old content only if preserving existing sessions.
> 2. Create the Lambda and HTTP API in the selected Region and attach Bedrock permissions to the **Lambda execution role**.
> 3. Configure Lambda environment variables, rebuild the ZIP, and deploy it.
> 4. Test the regional API directly before changing CloudFront.
> 5. Build the frontend using the new API URL, upload it to the selected frontend bucket, then change the CloudFront origin and invalidate `/*`.
>
> This order keeps the existing site available until the new backend has passed a direct API test.
...
Choose the Bedrock Region deliberately. For this project, use `eu-north-1` when deploying the Lambda in Stockholm. A cross-Region profile such as `eu.amazon.nova-2-lite-v1:0` can route within Europe; `deepseek.v3.2` is an in-Region model ID.
...
bedrock_client = boto3.client(
    ...
    region_name=os.getenv("BEDROCK_REGION", "eu-north-1")
)

# Bedrock model selection - see Q42 on https://edwarddonner.com/faq for more
BEDROCK_MODEL_ID = os.getenv("BEDROCK_MODEL_ID", "eu.amazon.nova-2-lite-v1:0").strip()
